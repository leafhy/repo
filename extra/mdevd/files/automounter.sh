...
my_umount() {
    # NOTE: busybox umount does not have a quiet option.
    #       util-linux umount supports the options (-q, --quiet).
...
my_mount() {
    ...
    case "${fstype:-auto}" in
        ntfs)           mount -t ntfs    -o ro,async             "/dev/$1" "$destdir/$1" || rmdir "$destdir/$1" ;;
                      # ntfs-3g          -o rw,uid=1000,gid=1000 "/dev/$1" "$destdir/$1" || rmdir "$destdir/$1" ;;
        xfs)            mount -t xfs     -o async                "/dev/$1" "$destdir/$1" || rmdir "$destdir/$1" ;;
        vfat)           mount -t vfat    -o async                "/dev/$1" "$destdir/$1" || rmdir "$destdir/$1" ;;
        f2fs)           mount -t f2fs    -o async                "/dev/$1" "$destdir/$1" || rmdir "$destdir/$1" ;;
        exfat)          mount -t exfat   -o async                "/dev/$1" "$destdir/$1" || rmdir "$destdir/$1" ;;
        ext2|ext3|ext4) mount -t $fstype -o async                "/dev/$1" "$destdir/$1" || rmdir "$destdir/$1" ;;
        hfs|hfsplus)    mount -t $fstype -o async                "/dev/$1" "$destdir/$1" || rmdir "$destdir/$1" ;;
        auto)           mount -t auto    -o async                "/dev/$1" "$destdir/$1" || rmdir "$destdir/$1" ;;
    esac
}
